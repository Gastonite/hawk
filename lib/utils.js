...
exports.parseHost = function (req, hostHeaderName) {
  ...
  if (hostHeaderName) {
    ...
    hostHeaderName = hostHeaderName.toLowerCase()

    if (req.headers[hostHeaderName]) {
  ...
  console.error('HawkUtils.parseHost.hostParts', hostParts)
  return {
    name: hostParts[1],
...
exports.parseRequest = function (req, options) {
  ...
  let url = req.url;
  if (options.pathHeaderName && req.headers[options.pathHeaderName]) {
    url = req.headers[options.pathHeaderName];
  }
  ...
  console.error('HawkUtils.parseRequest.request', request)
